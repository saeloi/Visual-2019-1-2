...
namespace A030_FormClock
{
    public partial class Form1 : Form
    {
        ...
        private void T_Tick(object sender, EventArgs e)
        {
            ...
            //초침의 길이는 200
            double x = 100 * Math.Sin(secDeg * Math.PI / 180); //(secDeg * Math.PI / 180)는 degree(secDeg)를 radian으로
            ...
            //분침의 길이는 100
            double x1 = 97 * Math.Sin(minDeg * Math.PI / 180); //(secDeg * Math.PI / 180)는 degree(secDeg)를 radian으로
            double y1 = 97 * Math.Cos(minDeg * Math.PI / 180);

            g.DrawLine(new Pen(Color.Red), new Point(150, 150),
                new Point((int)(150 + x1), (int)(150 - y1)));

            int hour = DateTime.Now.Hour; //분
            double hourDeg = hour * 30 + min*0.5; //시침이 12시 방향과 이루는 각도, min*0.5 10분이 5도
            //시침의 길이는 80
            double x2 = 80 * Math.Sin(hourDeg * Math.PI / 180); //(secDeg * Math.PI / 180)는 degree(secDeg)를 radian으로
